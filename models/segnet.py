...
class SegNet(nn.Module):
    ...
    """
    def __init__(self, num_classes, n_init_features=1, drop_rate=0.1,
        ...
        self.classifier = nn.Conv3d(filter_config[0], num_classes, 3, 1, 1)
...
class _Encoder(nn.Module):
    def __init__(self, n_in_feat, n_out_feat, n_blocks=2, drop_rate=0.5):
        ...
        layers = [nn.Conv3d(n_in_feat, n_out_feat, 3, 1, 1),
                  nn.BatchNorm3d(n_out_feat),
                  nn.ReLU(inplace=True)]

        if n_blocks > 1:
            layers += [nn.Conv3d(n_out_feat, n_out_feat, 3, 1, 1),
                       nn.BatchNorm3d(n_out_feat),
                       nn.ReLU(inplace=True)]
            if n_blocks == 3:
...
class _Decoder(nn.Module):
    ...
    def __init__(self, n_in_feat, n_out_feat, n_blocks=2, drop_rate=0.5):
        ...
        layers = [nn.Conv3d(n_in_feat, n_in_feat, 3, 1, 1),
                  nn.BatchNorm3d(n_in_feat),
                  nn.ReLU(inplace=True)]
